...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	if ll := len(key); ll != RecordRowKeyLen {
		return 0, 0, errInvalidRecordKey.GenWithStack("invalid key length")
	}

	if !hasTablePrefix(key) {
		return 0, 0, errInvalidRecordKey.GenWithStack("invalid table prefix")
	}
	...
	if !hasRecordPrefixSep(key) {
		return 0, 0, errInvalidRecordKey.GenWithStack("invalid record prefix")
	}
...
func DecodeIndexKeyPrefix(key kv.Key) (tableID int64, indexID int64, indexValues []byte, err error) {
	if ll := len(key); ll < prefixLen+idLen {
		return 0, 0, nil, errInvalidIndexKey.GenWithStack("invalid key length")
	}

	if !hasTablePrefix(key) {
		return 0, 0, nil, errInvalidRecordKey.GenWithStack("invalid table prefix")
	}
	...
	if !hasIndexPrefix(key) {
		return 0, 0, nil, errInvalidRecordKey.GenWithStack("invalid index prefix")
	}
